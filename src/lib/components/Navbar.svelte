...
<div class="navbar glass bg-gradient-to-tr from-black/50 to-zinc-700/50 h-15 grow left-1/2 -translate-x-1/2 top-0 z-50 fixed justify-center px-6 gap-4 sm:gap-8 md:gap-16 xl:gap-32 2xl:gap-80">
  <div class="navbar-start flex-none w-auto">
    {#if $isLoggedIn}
      <a
        class="hover:text-primary font-bold text-md md:text-xl flex items-center gap-2"
        href="#/"
    ...
    {:else}
      <a
        class="btn btn-primary btn-ghost text-md md:text-xl flex items-center"
        href="#/"
        on:click={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
        <img
          src="/server_nexus.ico"
          alt="Server Nexus logo"
          class="w-6 h-6 md:w-10 md:h-10 hover:animate-spin hover:[animation-duration:3s]"
          draggable="false" />
        <span>Server Nexus</span>
      </a>
    {/if}
  ...
  <div class="navbar-end flex-none w-auto">
    <div class="relative" bind:this={navDropdownEl}>
      ...
      {#if $isLoggedIn}
        {#if navMenuOpen}
          <ul class="menu menu-sm absolute left-0 glass bg-gradient-to-tr from-black/90 to-zinc-700/90 rounded-box z-50 mt-3 w-max whitespace-nowrap p-2 -mx-2">
            <li>
              <a
                class="text-lg whitespace-nowrap hover:bg-primary justify-center"
            ...
            <li>
              <a
                class="text-lg whitespace-nowrap hover:bg-primary"
    ...
    <div class="relative" bind:this={accountDropdownEl}>
      ...
      {#if $isLoggedIn}
        {#if accountMenuOpen}
          <ul class="menu menu-sm absolute right-0 glass bg-gradient-to-tr from-black/90 to-zinc-700/90 rounded-box z-50 mt-3 w-max whitespace-nowrap p-2 -mx-2 text-right">
            <li>
              <a
